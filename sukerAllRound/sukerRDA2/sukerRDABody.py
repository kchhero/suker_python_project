...
#            cmmFile.close()
    ...
    def getLinuxVersion(self, dirPath, binFileName) :
        try :
            with open(dirPath + getDirMark() + binFileName, 'rb') as ddrcs_bin :
                m = mmap.mmap(ddrcs_bin.fileno(), 0,access=mmap.ACCESS_READ)
                currPos = m.find(self.parsingListInKernel[0])
                if currPos == -1 :
                    return "Not Found Linux Banner!!"
                else :
                    m.seek(currPos)
                    readLine = m.read(128)
                    return readLine                        
    ...
    def getRamoops(self, dirPath, outPath, binFileName, address, size=RAMOOPS_SIZE) :
        try :
            with open(dirPath + getDirMark() + binFileName, 'rb') as ddrcs_bin :
                m = mmap.mmap(ddrcs_bin.fileno(), 0,access=mmap.ACCESS_READ)
                m.seek(address)
                readLine = m.read(size)                
                ramoopsOut = file(outPath, 'wt') 
                ramoopsOut.write(readLine)                  
                ramoopsOut.close()
